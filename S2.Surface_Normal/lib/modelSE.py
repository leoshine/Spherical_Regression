...
class VGG16_Trunk(nn.Module):
    ...
    def __init__(self, init_weights=True):
        ...
        self.deconv1_sgc = up([ 64,   4,         ], in_channels=64+64  ) # create_deconv_2(64+64, output_channel)
        self.deconv1_abs.deconv = self.deconv1_abs.deconv[:-1]  # [:-2]
        self.deconv1_sgc.deconv = self.deconv1_sgc.deconv[:-1]
        if init_weights:
            self.init_weights('torchmodel') # (None) #(pretrained='caffemodel' if (self.net_arch in ['vgg16', 'vgg19']) else None)
    ...
    def forward(self, x):  # In-replace forward_trunk
        ...
        #
        de4          = self.deconv5(en5, poolInd4, en4) # [1, 1024,  28,  28]
        de3          = self.deconv4(de4, poolInd3, en3) # [1,  512,  56,  56]
        de2          = self.deconv3(de3, poolInd2, en2) # [1,  256, 112, 112]
        de1          = self.deconv2(de2, poolInd1, en1) # [1,  128, 224, 224]
        # x            = self.deconv1(de1)
